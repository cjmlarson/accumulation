...
def raster(lat: float, lon: float, rp: int) -> Path:
    """Return the JRC depth raster containing a coordinate.
    ...
    directory = _DATA_DIR / f"RP{rp}"

    for latitude in (north, north - 10, north + 10):
        for longitude in (west, west - 10, west + 10):
            latitude_token = _coordinate(latitude, "N", "S")
            longitude_token = _coordinate(longitude, "E", "W")
            pattern = f"ID*_{latitude_token}_{longitude_token}_RP{rp}_depth.tif"
    ...
    raise FileNotFoundError(f"no JRC RP{rp} raster covers ({lat}, {lon})")
